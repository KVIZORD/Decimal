...
int s21_mul(s21_decimal value_1, s21_decimal value_2, s21_decimal *result) {
    ...
    printf("=> %d\n", get_width_number_in_bit(subtotal, 2 * INTS_IN_DECIMAL));
    return status;
...
int get_exponent(s21_decimal value) {
    return (value.bits[META_NUMBER_INT] << (BITS_IN_INT - 1 - BIT_EXP_END)) >> (BITS_IN_INT - 1 - BIT_EXP_END + BIT_EXP_START);
}

void set_exponent(s21_decimal* value, int exp) {
    int i = 0;
    value->bits[META_NUMBER_INT] &= ~((~i) << (BITS_IN_INT - 1 - BIT_EXP_END)) >> (BITS_IN_INT - 1 - BIT_EXP_END + BIT_EXP_START);
