...
int get_exp(int number) {
  return (number << (BITS_IN_INT - 1 - BIT_EXP_END)) >>
         (BITS_IN_INT - 1 - BIT_EXP_END + BIT_EXP_START);
}

int set_exp(int* number, int exp) {
  int i = 0;
  int mask_clean_exp =
      ~(~((~i) << (BIT_EXP_END - BIT_EXP_START + 1)) << BIT_EXP_START);
...
int get_sign(int number) { return get_bit_int(number, BIT_SIGN); }
